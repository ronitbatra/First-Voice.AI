      ...
      const finalGPT = await openai.chat.completions.create({
        ...
        messages: [
          ...
          })),
          {
            role: "system",
            content:
              `You are generating a structured mental health summary based on a conversation and relevant database responses.\n\n` +
              `Relevant DB info:\n${JSON.stringify(dbContext, null, 2)}\n\n` +
              `Now, return a summary using this **EXACT JSON format**:\n\n` +
              `{\n` +
              `  "triage": [\n` +
              `    "• 1) PRESENTING CONCERNS: [brief summary]",\n` +
              `    "• 2) EMOTIONAL STATE: [description]",\n` +
              `    "• 3) RISK FACTORS: [identified risks]",\n` +
              `    "•  4) SUPPORT NEEDS: [needs mentioned]",\n` +
              `    "•  5) RECOMMENDED CARE: [brief recommendations]"\n` +
              `  ],\n` +
              `  "openToHelp": true/false,\n` +
              `  "location": "location mentioned or 'their area' if unknown"\n` +
              `}\n\n` +
              `Respond ONLY in that JSON format. Keep the bullet points and the text in the same line.`,
          },
        ],
      });
